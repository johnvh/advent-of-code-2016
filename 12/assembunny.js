...
const initState = (ins, registers) => fromJS({
    i: 0,
    registers,
    nextIns: 0,
    ins
...
const assembunny = function*(initialState){
...
const run = (ins, registers = [0, 0, 0, 0]) => {
    let state = assembunny(initState(ins, registers));
    let s;
...
if(require.main === module){
    ...
    const r = run(input.map(parseIns), [0, 0, 0, 0]);
    console.dir(R.pick(['registers', 'nextIns', 'i'], r.toJS()), {colors: true});
}
